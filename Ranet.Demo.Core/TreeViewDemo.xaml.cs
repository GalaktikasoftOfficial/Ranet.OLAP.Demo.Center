...
namespace Ranet.Demo.Core
{
    public partial class TreeViewDemo
    {
        ...
        MemoryStream ScanForExeFile(string directory)
        {
        ...
        private void GetModules()
        {
            ...
            List<string> subdirectories = Directory.GetDirectories(basePath).ToList();
            subdirectories.Remove(basePath + ".vs");
            subdirectories.Remove(basePath + ".git");
            subdirectories.Remove(basePath + "Ranet.Demo.Core");
            subdirectories.Remove(basePath + "Ranet.DemoCenter");
            subdirectories.Remove(basePath + "BuildTools");
            subdirectories.Remove(basePath + "Docs");



            foreach (var subdirectory in subdirectories)
            {
                var dllStream = ScanForExeFile(subdirectory);
                if (dllStream == null)
                    continue;
